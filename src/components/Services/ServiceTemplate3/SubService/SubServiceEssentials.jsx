...
const SubServiceEssentials = ({ essentials = [] }) => {
  ...
  // Drag handlers for mobile swipe support
  ...
  return (
    <section className="py-20 bg-slate-900 text-white overflow-hidden relative border-b border-slate-950">
      {/* Decorative Glow */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={handlePrev}
              className="w-12 h-12 rounded-full border border-white/10 bg-white/5 hover:bg-white/10 hover:border-blue-400/40 text-slate-300 transition-all duration-300 flex items-center justify-center focus:outline-none"
          ...
          >
            {essentials.map((item, idx) => (
              <div
                ...
                style={{ width: "100%" }}
              >
                <div className="bg-slate-950/80 border border-white/5 p-8 sm:p-12 rounded-3xl backdrop-blur-md shadow-2xl flex flex-col justify-between min-h-[300px]">
                  <div>
          ...
          {essentials.map((_, idx) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                currentIdx === idx ? "w-8 bg-blue-500" : "w-2 bg-slate-700 hover:bg-slate-650"
              }`}

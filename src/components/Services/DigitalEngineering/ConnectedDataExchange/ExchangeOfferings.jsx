...
const SERVICES = [
  {
    title: "Event-Driven Brokering",
    desc: "Configure Apache Kafka, RabbitMQ, or AWS EventBridge pipelines to route transaction messages at scale.",
    featured: true,
  },
  { title: "Schema Registry Validation", desc: "Enforce payload rules at broker entry nodes using Apache Avro or JSON schema controls." },
  { title: "Dead-Letter Queues & Retries", desc: "Isolate anomalous or malformed payloads into dead-letter storage lanes while retrying valid ones." },
  { title: "High-Volume Stream Buffering", desc: "Configure temporary queue caches to buffer traffic spikes, avoiding database crashes." },
  { title: "Transaction Replay Pipelines", desc: "Build replayable log systems enabling developers to recreate database states during auditing." },
  { title: "End-to-End Delivery Monitoring", desc: "Install Prometheus and Grafana alerts capturing message delivery rates, queue lags, and broker failures." },
];

const ExchangeOfferings = () => {
  return (
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-14">
        ...
        >
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-medium text-black mb-4">
            Our Connected Data Exchange Offerings
          </h2>
          <p className="text-lg sm:text-xl font-light text-[#7b7b7b]">
            Architect stream networks, validate payloads dynamically, and implement monitoring.
          </p>
        </motion.div>
          ...
          >
            ...
            <div className="relative z-10 flex h-full flex-col justify-between p-8 sm:p-10">
              ...
              <p className="text-xl font-medium leading-snug text-white sm:text-2xl">
                Structuring resilient queues to maintain message ordering across global server clusters.
              </p>
            </div>
          </motion.div>
        </div>
...
export default ExchangeOfferings;

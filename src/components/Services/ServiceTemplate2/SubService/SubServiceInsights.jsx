...
const SubServiceInsights = ({ insights = [] }) => {
  ...
  return (
    <section className="py-20 bg-white text-slate-800 border-b border-slate-200/50 relative">
        ...
        >
          {insights.map((item, idx) => (
            <motion.div
              ...
              className="group border border-slate-200 rounded-[28px] overflow-hidden bg-white shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col"
            >
              {/* Image Header */}
              {item.image && (
                <div className="aspect-[21/9] w-full overflow-hidden relative bg-slate-50">
                  <img
                    src={item.image}
                    alt={item.title}
